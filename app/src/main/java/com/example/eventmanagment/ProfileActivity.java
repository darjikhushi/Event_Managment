...
//
//        // Enable editing
//        nameLayout.setEndIconOnClickListener(v -> edtName.setEnabled(true));
//        mobileLayout.setEndIconOnClickListener(v -> edtMobile.setEnabled(true));
//        emailLayout.setEndIconOnClickListener(v -> edtEmail.setEnabled(true));
//
//        // Update
//        btnUpdateProfile.setOnClickListener(v -> updateProfile());
//
...
public class ProfileActivity extends AppCompatActivity {
    ...
    private void updateProfile() {
        ...
        if (!newName.matches("^[a-zA-Z ]+$")) {
            edtName.setError("Name should contain only letters");
            edtName.requestFocus();
            return;
        }
        ...
        // Update email in Firebase Auth
        user.updateEmail(newEmail).addOnCompleteListener(task -> {
            if (!task.isSuccessful()) edtEmail.setError("Failed to update email");
        });
        ...
        if (!oldPass.isEmpty() && !newPass.isEmpty()) {
            ...
            if (!newPass.matches(".*[A-Z].*")) {
                edtNewPassword.setError("Password must contain at least 1 uppercase letter");
                edtNewPassword.requestFocus();
                return;
            }
            if (!newPass.matches(".*[0-9].*")) {
                edtNewPassword.setError("Password must contain at least 1 number");
                edtNewPassword.requestFocus();
                return;
            }
            if (!newPass.matches(".*[!@#$%^&*()._+=-].*")) {
                edtNewPassword.setError("Password must contain 1 special character");
                edtNewPassword.requestFocus();
                return;
            }
                ...
                if (task.isSuccessful()) {
                    user.updatePassword(newPass).addOnCompleteListener(passTask -> {
                        if (!passTask.isSuccessful()) {
                            edtNewPassword.setError("Failed to update password");
                        }
                    });
                } else {
